...
export default function ChannelsLink(props) {
    return (
        <a className={`nav-link ${props.current ? 'active': ''}`}  href={props.link}>
            ...
            {props.name} { props.current && <span className="sr-only">(current)</span>}         
        </a>
    )
}